...
| Full regression | `npm.cmd test -- --run`: 169 files, 877 tests; 1 MySQL integration test separately passed | Complete |
...
| Operating query efficiency | Migrations 0036-0039; 18 schema-aligned operating indexes plus the stable application-ledger cursor index; indexed due-alert selection with paged canonical-job and application-ledger reads; bounded privacy/deduplication/admin-review reads; exact owned lifecycle/admin evidence/approval/preparation/decision-close/employer-reply/interview-context queries; application-scoped approval reads; exact verification-review resolution; shared child evidence for the ledger and autonomous follow-ups; preloaded offer attribution and interview-preparation datasets | Complete locally; the 40-entry migration contract is checked and the prior 39-entry history was applied from zero on MySQL 8.4; current clean-database CI acceptance is required for migration 0039 |
