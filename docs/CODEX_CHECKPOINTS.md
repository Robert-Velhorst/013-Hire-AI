...
| Full regression | `npm.cmd test -- --run`: 159 files, 817 tests | Complete |
...
| Operating query efficiency | Migration 0036; 17 schema-aligned indexes; bounded privacy/deduplication/admin-review reads; exact owned lifecycle/admin evidence/approval/preparation/decision-close/employer-reply/interview-context queries; application-scoped approval reads; exact verification-review resolution; shared child evidence, offer attribution, and interview-preparation datasets per ledger | Complete locally; production migration acceptance pending |
