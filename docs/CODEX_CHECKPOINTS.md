...
| Full regression | `npm.cmd test -- --run`: 169 files, 883 tests; 1 MySQL integration test separately passed | Complete |
...
| Operating query efficiency | Migrations 0036-0040; 18 schema-aligned operating indexes plus stable ledger and active-work indexes; indexed due-alert selection with paged canonical-job and application-ledger reads; bounded dashboard/autonomous application, approval, and review-decision workloads; exact aggregate totals, current-job history, and daily preparation counts; exact owned lifecycle/admin evidence/approval/preparation/decision-close/employer-reply/interview-context queries; shared child evidence for the bounded operating set | Complete locally with synthetic 260-application/105-approval evidence; the prior 40-entry history passed clean-container CI and current 41-entry acceptance is required for migration 0040 |
