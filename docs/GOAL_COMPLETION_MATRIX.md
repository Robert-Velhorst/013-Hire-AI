...
| 028-030 privacy, web security, secrets | Partial | Authenticated metadata export, user-owned deletion-review intake/cancellation, admin retention decisions, audit attribution, and baseline headers exist. Approved retention rules and verified erasure execution remain. |
| 031-036 local dev, Docker, migrations, doctor, health, diagnostics | Partial | Dev scripts, migration journal, doctor, health/readiness, and Dockerfile exist. Container build and production database checks remain environment work. |
...
| 040-052 tests, adversarial/ownership, provider failure, accessibility, scale | Partial | Broad unit/integration suite exists. Browser accessibility, full E2E, and large database load testing remain to be run in a deployed environment. |
...
| 092-104 clone, verification, roadmap, provider cleanup, support, retention, migration, emergency stop | Partial | Commands, runbooks, and deletion-review intake exist; fresh-clone, backup restore, approved retention schedules, verified erasure execution, and real-provider cleanup await environment and owner action. |
| 105-115 onboarding, permissions, scoring, exceptions, retries, versioning, final operator test | Partial | Profile readiness, confidence/safety signals, review queue, idempotency, and migration history exist. Team permissions and human operator validation remain product/operational work. |
